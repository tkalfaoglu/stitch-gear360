...
while test $# -gt 1; do
    ...
    echo  ffmpeg ${FFMPEG_OPTS}  -i $1 -vf scale=3840:1920  -c:a copy  -crf 22 -preset ultrafast resized/$1
          ffmpeg ${FFMPEG_OPTS}  -i $1 -vf scale=3840:1920  -c:a copy  -crf 22 -preset ultrafast resized/$1
    shift
done
...
echo ffmpeg  -f concat ${FFMPEG_OPTS} -safe 0  -i "${CACHE_DIR}/ffmpeg-queue.txt" "${CACHE_DIR}/ffmpeg-output.mp4"
     ffmpeg  -f concat ${FFMPEG_OPTS} -safe 0  -i "${CACHE_DIR}/ffmpeg-queue.txt" "${CACHE_DIR}/ffmpeg-output.mp4"
if ! [ $? -eq 0 ]; then
